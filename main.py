...
def system():
    ...
    #loading test data
    df_test = pd.read_csv('bank/bank-additional.csv', sep=';')
    ...
    return {
        'bias':format(avg_bias, '.2f'),
        'variance':format(avg_var, '.2f'),
        'accuracy_in_real':format(accuracy_of_real_data, '.2f'),
        'loss':format(avg_loss,'.2f'),
        'matrix':matrix,
    }
